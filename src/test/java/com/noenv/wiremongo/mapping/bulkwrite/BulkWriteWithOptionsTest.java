...
public class BulkWriteWithOptionsTest extends TestBase {
  ...
  public void testBulkWriteWithOptionsFileReturnedObjectNotModified(TestContext ctx) {
    ...
    db.bulkWriteWithOptions("bulkwritewithoptions", Arrays.asList(BulkOperation.createInsert(new JsonObject().put("test", "testBulkWriteWithOptionsFile"))), new BulkWriteOptions().setWriteOption(WriteOption.ACKNOWLEDGED))
      .onSuccess(actual -> ctx.assertEquals(expected.toJson(), actual.toJson()))
      .onSuccess(actual -> {
        actual.getUpserts().get(0).put("field1", "replace");
        actual.getUpserts().get(0).put("add", "add");
