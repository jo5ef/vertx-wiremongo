...
public class VerificationTest extends TestBase {
  ...
  public void test_shall_fail_ifDefinedVerificationIsNotRun() {
    ...
    mock.findOneAndUpdate()
      ...
      .verify(verifier.checkIf("find one and update in some-collection")
        .isRunExactlyOnce())
      .returns(null);
  }
  ...
  public void verify_RunExactlyOnce_shall_succeed_ifRunOnce(TestContext ctx) {
    ...
    db.findOneAndUpdate("some-collection", new JsonObject(), new JsonObject())
  ...
  public void verify_RunAtLeastOnce_shall_succeed_ifRunOnce(TestContext ctx) {
    mock.updateCollection()
      .verify(verifier.checkIf("update in any collection")
        .isRunAtLeastOnce())
      .returns(null);

    db.updateCollection("any-collection", new JsonObject(), new JsonObject())
      .onComplete(ctx.asyncAssertSuccess());
  }
  ...
  public void verify_RunAtLeastOnce_shall_succeed_ifRunTwice(TestContext ctx) {
    ...
    CompositeFuture.all(
      db.updateCollection("any-collection-1", new JsonObject(), new JsonObject()),
      db.updateCollection("any-collection-2", new JsonObject(), new JsonObject()))
      .onComplete(ctx.asyncAssertSuccess());
  }
  ...
  public void verifyOrder_shall_succeed_ifRunInExpectedOrder(TestContext ctx) {
    mock.updateCollection()
      ...
      .verify(verifier.checkIf("update in collection-1")
        .isRunAfterPreviousVerifications())
      .returns(null);

    mock.updateCollection()
      .inCollection("collection-2")
      .verify(verifier.checkIf("update in collection-2")
        .isRunAfterPreviousVerifications())
      .returns(null);

    CompositeFuture.all(
      db.updateCollection("collection-1", new JsonObject(), new JsonObject()),
      db.updateCollection("collection-2", new JsonObject(), new JsonObject()))
      .onComplete(ctx.asyncAssertSuccess());
  }
